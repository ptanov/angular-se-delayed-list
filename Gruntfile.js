...
module.exports = function (grunt) {
	...
	function initConfig() {
		function getValidateTasks(allTasks) {
			if (process.env.SEDELAYEDLIST_FAST === "true") {
				return [];
			}
			if (process.env.SEDELAYEDLIST_W3C_LOCAL_URL && allTasks) {
				return [
					"jshint",
					"karma:unit",
					"htmlangular:index",
		...
		grunt.initConfig({
			...
			htmlangular: {
				options: {
					...
					w3clocal: process.env.SEDELAYEDLIST_W3C_LOCAL_URL?process.env.SEDELAYEDLIST_W3C_LOCAL_URL:null,
					//i'm not sure why in local instalation these errors exists:
					relaxerror: process.env.SEDELAYEDLIST_W3C_LOCAL_URL?[
	...
	function getBrowsersForTest() {
		if (process.env.SEDELAYEDLIST_BROWSERS_FOR_TEST) {
			return process.env.SEDELAYEDLIST_BROWSERS_FOR_TEST.split(",");
		} else {
